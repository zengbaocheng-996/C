...
{
    LNode *p = GetElem(L,i-1);//拿到要删除结点的前一个结点
    if(NULL==p)
    {
        return false;
    }
    LNode *q = p->next;//拿到要删除的结点指针
    if(NULL==q)//当链表只有5个结点，删除第6个结点，出现这种异常情况，避免程序崩溃
    {
        return false;
    }
    p->next = q->next;//断链
...
//    list_head_insert(L);//输入数据可以为3，4，5，6，7，9999，头插法新建链表
    list_tail_insert(L);
    print_list(L);
//    bool ret = ListFrontInsert(L,1,99);//新结点插入第i个位置
//    if(ret)
//    {
//        print_list(L);//打印链表
//    }
